...
# Полная организационно-правовая форма (как пишет ФНС в начале
# НаимПолнЮЛ) -> общепринятое сокращение. Порядок важен: более
# специфичные формы («ПУБЛИЧНОЕ АКЦИОНЕРНОЕ ОБЩЕСТВО») проверяются раньше
# их более общих подстрок («АКЦИОНЕРНОЕ ОБЩЕСТВО»)
_LEGAL_FORM_ABBR = [
...
def _display_company_name(short_name: Optional[str], company_name: Optional[str]) -> str:
    """
    Короткое узнаваемое название для компактных списков (дашборд), в
    привычном формате 'ООО "Название"': организационно-правовая форма
    сворачивается до общепринятого сокращения, а само название берется
    из кавычек в company_name. Полное юрлицо ("ОБЩЕСТВО С ОГРАНИЧЕННОЙ
    ОТВЕТСТВЕННОСТЬЮ «Ромашка»") — более надежный источник для этого,
    чем short_name: он есть у всех отчетов (включая созданные до
    появления поля short_name) и форма в нем всегда написана полностью,
    без сокращений вроде ФНС-овского "ООО" перед кавычками не всегда.

    Для ИП/физлиц (нет организационно-правовой формы и кавычек в имени)
    возвращается как есть, без обертки.
    """
    source = company_name or short_name
    if not source:
        return "Неизвестно"

    upper = source.upper()
    abbr = next((short for full, short in _LEGAL_FORM_ABBR if upper.startswith(full)), None)

    match = _QUOTED_NAME_RE.search(source)
